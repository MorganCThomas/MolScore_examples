#!/usr/bin/env python3
""" Adapted to include MolScore from
...
def main(args):
    ...
    if (args.molscore in MolScoreBenchmark.presets):
        scoring_function = MolScoreBenchmark(model_name='CReM', output_dir="./", budget=10000, benchmark=args.molscore)
        for task in scoring_function:
            best = optimiser.generate_optimized_molecules(scoring_function=task)
    
    elif os.path.isdir(args.molscore):
        scoring_function = MolScoreBenchmark(model_name='CReM', output_dir="./", budget=10000, custom_benchmark=args.molscore)
        for task in scoring_function:
            best = optimiser.generate_optimized_molecules(scoring_function=task)

    else:
        scoring_function = MolScore(model_name='CReM', task_config=args.molscore)
